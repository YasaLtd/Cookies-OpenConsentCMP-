...
Do not put the version number in the installable ZIP filename. A root-packed ZIP installs under the ZIP basename, so a filename such as `openconsent-cmp-wordpress-plugin-1.1.7.zip` creates the wrong plugin slug and makes Plugin Check expect the wrong text domain. Keep the installable filename `openconsent-cmp.zip`; put release versioning in plugin metadata, `readme.txt`, Git tags, website visible labels, and cache-busting query strings such as `openconsent-cmp.zip?v=1.1.7`.

Build the ZIP with forward-slash internal paths only, even on Windows. Do not use PowerShell `Compress-Archive` for the release ZIP because it can create entries such as `includes\class-openconsent-cmp.php`; those are not WordPress-safe on Linux hosts. The ZIP entries must be `includes/class-openconsent-cmp.php`, `assets/js/openconsent-cmp.js`, and so on.
...
3. ZIP paths use forward slashes only; no entry may contain a backslash.
...
6. Frontend-affecting admin settings round-trip: save them in local WordPress, verify the stored option, verify the localized `OpenConsentCMP.ui` config in frontend HTML, and verify visible frontend DOM/CSS behavior.
7. The admin settings page has no nested forms. Verify the main `options.php` form contains the Save Changes button and no child `<form>` tags before release.
8. The official WordPress Plugin Check plugin reports no errors for the installed release ZIP. Run WP-CLI from the clean WordPress root so it scans `wp-content/plugins/openconsent-cmp`, not this development repository.
9. The live website download link returns the same ZIP hash as the local package.
